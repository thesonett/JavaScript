 ...
 * Description
 */
export function generalPartition(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => operations.PartitionRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    if (isBlobLike(payload.partition_parameters.files)) {
        ...
        const buffer = await readableStreamToArrayBuffer(payload.partition_parameters.files.content);
        const blob = new Blob([buffer], { type: "application/octet-stream" });
        appendForm(body, "files", blob);
    }
    else {
        appendForm(body, "files", new Blob([payload.partition_parameters.files.content], {
            type: "application/octet-stream",
        }), payload.partition_parameters.files.fileName);
    }
    if (payload.partition_parameters.chunking_strategy !== undefined) {
        appendForm(body, "chunking_strategy", payload.partition_parameters.chunking_strategy);
    }
    if (payload.partition_parameters.combine_under_n_chars !== undefined) {
        appendForm(body, "combine_under_n_chars", payload.partition_parameters.combine_under_n_chars);
    }
    ...
    if (payload.partition_parameters.extract_image_block_types !== undefined) {
        appendForm(body, "extract_image_block_types", payload.partition_parameters.extract_image_block_types);
    }
    if (payload.partition_parameters.gz_uncompressed_content_type !== undefined) {
        appendForm(body, "gz_uncompressed_content_type", payload.partition_parameters.gz_uncompressed_content_type);
    }
    if (payload.partition_parameters.hi_res_model_name !== undefined) {
        appendForm(body, "hi_res_model_name", payload.partition_parameters.hi_res_model_name);
    }
    if (payload.partition_parameters.include_orig_elements !== undefined) {
        appendForm(body, "include_orig_elements", payload.partition_parameters.include_orig_elements);
    }
    if (payload.partition_parameters.include_page_breaks !== undefined) {
        appendForm(body, "include_page_breaks", payload.partition_parameters.include_page_breaks);
    }
    if (payload.partition_parameters.include_slide_notes !== undefined) {
        appendForm(body, "include_slide_notes", payload.partition_parameters.include_slide_notes);
    }
    ...
    if (payload.partition_parameters.max_characters !== undefined) {
        appendForm(body, "max_characters", payload.partition_parameters.max_characters);
    }
    if (payload.partition_parameters.multipage_sections !== undefined) {
        appendForm(body, "multipage_sections", payload.partition_parameters.multipage_sections);
    }
    if (payload.partition_parameters.new_after_n_chars !== undefined) {
        appendForm(body, "new_after_n_chars", payload.partition_parameters.new_after_n_chars);
    }
    if (payload.partition_parameters.ocr_languages !== undefined) {
        appendForm(body, "ocr_languages", payload.partition_parameters.ocr_languages);
    }
    if (payload.partition_parameters.output_format !== undefined) {
        appendForm(body, "output_format", payload.partition_parameters.output_format);
    }
    ...
    if (payload.partition_parameters.pdf_infer_table_structure !== undefined) {
        appendForm(body, "pdf_infer_table_structure", payload.partition_parameters.pdf_infer_table_structure);
    }
    if (payload.partition_parameters.pdfminer_char_margin !== undefined) {
        appendForm(body, "pdfminer_char_margin", payload.partition_parameters.pdfminer_char_margin);
    }
    if (payload.partition_parameters.pdfminer_line_margin !== undefined) {
        appendForm(body, "pdfminer_line_margin", payload.partition_parameters.pdfminer_line_margin);
    }
    if (payload.partition_parameters.pdfminer_line_overlap !== undefined) {
        appendForm(body, "pdfminer_line_overlap", payload.partition_parameters.pdfminer_line_overlap);
    }
    if (payload.partition_parameters.pdfminer_word_margin !== undefined) {
        appendForm(body, "pdfminer_word_margin", payload.partition_parameters.pdfminer_word_margin);
    }
    if (payload.partition_parameters.similarity_threshold !== undefined) {
        appendForm(body, "similarity_threshold", payload.partition_parameters.similarity_threshold);
    }
    if (payload.partition_parameters.skip_infer_table_types !== undefined) {
        appendForm(body, "skip_infer_table_types", payload.partition_parameters.skip_infer_table_types);
    }
    if (payload.partition_parameters.split_pdf_allow_failed !== undefined) {
        appendForm(body, "split_pdf_allow_failed", payload.partition_parameters.split_pdf_allow_failed);
    }
    if (payload.partition_parameters.split_pdf_concurrency_level !== undefined) {
        appendForm(body, "split_pdf_concurrency_level", payload.partition_parameters.split_pdf_concurrency_level);
    }
    if (payload.partition_parameters.split_pdf_page !== undefined) {
        appendForm(body, "split_pdf_page", payload.partition_parameters.split_pdf_page);
    }
    if (payload.partition_parameters.split_pdf_page_range !== undefined) {
        appendForm(body, "split_pdf_page_range", payload.partition_parameters.split_pdf_page_range);
    }
    if (payload.partition_parameters.starting_page_number !== undefined) {
        appendForm(body, "starting_page_number", payload.partition_parameters.starting_page_number);
    }
    ...
    if (payload.partition_parameters.table_ocr_agent !== undefined) {
        appendForm(body, "table_ocr_agent", payload.partition_parameters.table_ocr_agent);
    }
    if (payload.partition_parameters.unique_element_ids !== undefined) {
        appendForm(body, "unique_element_ids", payload.partition_parameters.unique_element_ids);
    }
    ...
    if (payload.partition_parameters.vlm_model_provider !== undefined) {
        appendForm(body, "vlm_model_provider", payload.partition_parameters.vlm_model_provider);
    }
    if (payload.partition_parameters.xml_keep_tags !== undefined) {
        appendForm(body, "xml_keep_tags", payload.partition_parameters.xml_keep_tags);
    }
    const path = pathToFunc("/general/v0/general")();
    const headers = new Headers(compactMap({
        Accept: options?.acceptHeaderOverride
            || "application/json;q=1, text/csv;q=0",
        "unstructured-api-key": encodeSimple("unstructured-api-key", payload["unstructured-api-key"], { explode: false, charEncoding: "none" }),
    }));
    ...
    const [result] = await M.match(M.json(200, operations.PartitionResponse$inboundSchema), M.text(200, operations.PartitionResponse$inboundSchema, {
        ctype: "text/csv",
